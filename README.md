...
Results:
  ...
  First run:  2 HTML beleg (resolve nav params) + 2 HTML sentence + 1 HTML tree = 5 total
  If belags already cached: 0 + 2 + 1 = 3 total
  On full repeat: 0 (all cached)
```
...
결과:
  ...
  최초 실행: JSON 검색 2회 + HTML 토큰 상세 5회 = 총 7회
  책이 이미 캐시된 경우: JSON 0회 + HTML 5회 = 총 5회
  전체 반복 시: 0회 (전부 캐시)
```
...
결과:
  ...
  최초 실행: HTML beleg 2회 (내비게이션 파라미터 해석) + HTML 문장 2회 + HTML 트리 1회 = 총 5회
  beleg이 이미 캐시된 경우: 0 + 2 + 1 = 총 3회
  전체 반복 시: 0회 (전부 캐시)
```
